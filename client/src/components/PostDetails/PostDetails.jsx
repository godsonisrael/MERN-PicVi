  ...
  return (
    <Paper elevation={6} style={{ padding: '20px', borderRadius: '15px' }} >
      ...
      {recommendedPosts.length ? (
        <div className={classes.section}>
          <Typography variant="h5" gutterBottom>Suggested posts</Typography>
          <Divider />
          <div className={classes.recommendedPosts}>
            {recommendedPosts.map(({  title, name, likes, selectedFile, _id }) => (
              <div style={{  margin: '20px', cursor: 'pointer' }} onClick={() => openPost(_id)} key={_id}>
                <Typography gutterBottom variant="h6">{title}
                  <div><Typography gutterBottom variant="subtitle2">posted by {name}</Typography></div>
                </Typography>
